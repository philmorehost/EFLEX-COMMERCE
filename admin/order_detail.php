...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($new_status) && $new_status !== $old_status){
        // If the order is being marked as completed, deduct stock
        if ($new_status === 'Completed') {
            ...
            if($stmt_items_stock = $mysqli->prepare($sql_items_for_stock)){
                ...
                $items_to_deduct = $stmt_items_stock->get_result()->fetch_all(MYSQLI_ASSOC);
                ...
                foreach ($items_to_deduct as $item) {
                 ...
                 $message .= '<div class="alert alert-success">Stock has been deducted for all items in the order.</div>';
            }
        }
